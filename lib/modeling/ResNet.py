    ...
    def __init__(self, block_counts):
        ...
        if cfg.MODEL.USE_DEFORM:
            self.res3, dim_in = add_stage(dim_in, 512, dim_bottleneck * 2, block_counts[1],
                                          dilation=1, stride_init=2, deform=True, nres=3)
            self.res4, dim_in = add_stage(dim_in, 1024, dim_bottleneck * 4, block_counts[2],
                                          dilation=1, stride_init=2, deform=True, nres=4)
        else:
            self.res3, dim_in = add_stage(dim_in, 512, dim_bottleneck * 2, block_counts[1],
        ...
        if len(block_counts) == 4:
            ...
            if cfg.MODEL.USE_DEFORM:
                self.res5, dim_in = add_stage(dim_in, 2048, dim_bottleneck * 8, block_counts[3],
                                              cfg.RESNETS.RES5_DILATION, stride_init, deform=cfg.MODEL.USE_DEFORM, nres=5)
            else:
                self.res5, dim_in = add_stage(dim_in, 2048, dim_bottleneck * 8, block_counts[3],
                                              cfg.RESNETS.RES5_DILATION, stride_init, deform=cfg.MODEL.USE_DEFORM)
