    ...
    for lvl in range(cfg.FPN.RPN_MIN_LEVEL, cfg.FPN.RPN_MAX_LEVEL + 1):
        ...
        if cfg.RPN.CLS_ACTIVATION == 'softmax':
            ...
            if not cfg.RPN.FL_INIT:
                ...
                alpha = torch.ones(2) * cfg.FAST_RCNN.ALPHA
                alpha[0] = 1 - cfg.FAST_RCNN.ALPHA
                from lib.modeling.FocalLoss import FocalLoss
                focalloss = FocalLoss(class_num=2, alpha=alpha, gamma=cfg.FAST_RCNN.GAMMA)
                rpn_cls_logits = kwargs['rpn_cls_logits_fpn' + slvl].contiguous()
                rpn_cls_logits = rpn_cls_logits[rpn_labels_int32_fpn >= .0]
                rpn_cls_logits = F.sigmoid(rpn_cls_logits)
                new_rpn_cls_logits = torch.zeros(rpn_cls_logits.shape[0], 2).cuda()
                rpn_labels_int32_fpn = rpn_labels_int32_fpn[rpn_labels_int32_fpn >= .0].long()
                rows = rpn_cls_logits.shape[0]
                b4 = np.array(range(rows))
                new_rpn_cls_logits[b4, rpn_labels_int32_fpn.view(rpn_labels_int32_fpn.numel())] = rpn_cls_logits

                new_rpn_labels_int32_fpn = torch.where(rpn_labels_int32_fpn == .0, torch.full_like(rpn_labels_int32_fpn, 1),
                                                       torch.full_like(rpn_labels_int32_fpn, 0)).long()

                new_rpn_cls_logits[b4, new_rpn_labels_int32_fpn.view(new_rpn_labels_int32_fpn.numel())] = 1 - rpn_cls_logits

                if len(new_rpn_cls_logits) == 0:
                    loss_rpn_cls_fpn = torch.zeros(1)[0].cuda()
                else:
                    loss_rpn_cls_fpn = focalloss(new_rpn_cls_logits.cuda(), rpn_labels_int32_fpn)
                # loss_rpn_cls_fpn /= cfg.TRAIN.RPN_BATCH_SIZE_PER_IM * cfg.TRAIN.IMS_PER_BATCH
