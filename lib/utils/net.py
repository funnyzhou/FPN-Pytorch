...
def update_learning_rate(optimizer, cur_lr, new_lr):
    ...
    if cur_lr != new_lr:
        ...
        if cfg.FAST_RCNN.USE_CASCADE:
            for ind, param_group in enumerate(optimizer.param_groups):
                if ind == 3:
                    param_group['lr'] = new_lr / cfg.CASCADE_RCNN.STAGE_WEIGHTS[0]
                elif ind == 4:
                    param_group['lr'] = new_lr / cfg.CASCADE_RCNN.STAGE_WEIGHTS[1]
                elif ind == 5:
                    param_group['lr'] = new_lr / cfg.CASCADE_RCNN.STAGE_WEIGHTS[2]
