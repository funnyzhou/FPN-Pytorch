    ...
    if len(gt_boxes) > 0:
        ...
        if cfg.RPN.SNIP:
            for i in range(len(gt_boxes)):
                ...
                RES = np.sqrt(width * height)
                if not (RES > cfg.RPN.RES_LO and RES <= cfg.RPN.RES_HI):
                    ids = np.where(anchor_to_gt_argmax == i)[0]
                    for id in ids:
                        if anchor_to_gt_max[id] > cfg.RPN.SNIP_NEG_THRESH:
                            # create an exception (neither fg/bg)
                            anchor_to_gt_max[id] = cfg.RPN.SNIP_TARGET_THRESH

            labels[anchors_with_max_overlap] = 1
            # Fg label: above threshold IOU
            labels[anchor_to_gt_max >= cfg.TRAIN.RPN_POSITIVE_OVERLAP] = 1
        else:
            if gt_boxes[0][2] - gt_boxes[0][0] > 0.1:
                labels[anchors_with_max_overlap] = 1
            # Fg label: above threshold IOU
            labels[anchor_to_gt_max >= cfg.TRAIN.RPN_POSITIVE_OVERLAP] = 1
